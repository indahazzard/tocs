...
//        int keyToSearch = 10 * 11;
        ...
        for (int size : sizes) {
            ...
            long interpolationStart = System.currentTimeMillis();
            for (int i = 0; i <= forInterpolation.length -1; i++) {
                ...
                if (foundsCountBinAvg[0] != -1) {
                    System.out.println("inter "+interpolationIndex[0]);
                    foundsCountInterAvg[i] = interpolationIndex[0];
                }
//                System.out.println("Index inter: " + interpolationIndex[0]);
            }
//            System.out.println("comparesCountInterAvg: "+Arrays.toString(comparesCountInterAvg));
//            System.out.println("iterationCountInterAvg: "+Arrays.toString(iterationCountInterAvg));
            long interpolationEnd = System.currentTimeMillis();
            ...
            for (int j = 0; j <= forBinary.length -1; j++) {
                ...
                int[] binaryIndex = binarySearch(forBinary, 0, forBinary.length - 1, forBinary[j], comparesCountBin, iterationCountBin);
                comparesCountBinAvg[j] = binaryIndex[1];
                iterationCountBinAvg[j] = binaryIndex[2];
//                System.out.println("Index binary: " + binaryIndex[0]);
                if (foundsCountBinAvg[0] != -1) {
                    System.out.println("bin: "+foundsCountBinAvg[0]);
                    foundsCountInterAvg[j] = foundsCountBinAvg[0];
                }
            }
            long binaryEnd = System.currentTimeMillis();
//            System.out.println("comparesCountBinAvg: "+Arrays.toString(comparesCountBinAvg));
//            System.out.println("iterationCountBinAvg: "+Arrays.toString(iterationCountBinAvg));
    ...
    public static int[] interpolationSearch(int[] arr, int lowest, int highest, int x, int comparesCount, int iterationCount) {
        ...
        if (lowest <= highest && x >= arr[lowest] && x <= arr[highest]) {
            ...
            position = lowest + (((highest - lowest) / (arr[highest] - arr[lowest])) * (x - arr[lowest]));
